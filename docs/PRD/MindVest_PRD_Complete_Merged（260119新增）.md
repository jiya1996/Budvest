...
|----|---------|--------|--------|---------|
| 1.0 | 用户系统 | **P0** | 静默登录 + 游客模式 | openid登录，游客UUID |
...
| 3.0 | 持仓管理 | **P0** | 手动维护持仓 | 手动输入代码+数量+成本,语音P2 |
...
| 功能 | MVP做 | MVP不做 |
|------|--------|---------|
| 登录 | openid静默登录 + 游客UUID | 昵称头像授权、手机号登录 |
...
sequenceDiagram
    participant U as 用户
    participant MP as 链接
    participant WX as web端H5界面
