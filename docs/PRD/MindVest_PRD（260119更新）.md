...
|----|---------|--------|--------|---------|
| 1.0 | 用户系统 | **P0** | 邮箱/手机登录 + 游客模式 | 邮箱/手机登录，游客UUID |
...
| 功能 | MVP做 | MVP不做 |
|------|--------|---------|
| 登录 | 游客模式 + 邮箱/手机登录 | OAuth授权登录（Google/Apple） |
...
sequenceDiagram
    ...
    alt 有本地Token
        SW-->>PWA: 返回Token
        PWA->>API: POST /api/auth/verify {token}
        API->>DB: 验证Token
        API-->>PWA: {valid: true, userId}
        PWA->>U: 显示首页
    else 无Token/Token过期
        PWA->>U: 显示登录页
        U->>PWA: 邮箱/手机登录
        PWA->>API: POST /api/auth/login {credentials}
        API->>DB: 验证并创建Session

        alt 新用户
            API->>DB: 创建用户记录
            API-->>PWA: {token, isNewUser: true, userId}
            PWA->>SW: 存储Token
            PWA->>U: 显示初心设定页
        else 老用户
            API-->>PWA: {token, isNewUser: false, userId}
            PWA->>SW: 存储Token
            PWA->>U: 显示首页（无引导）
        end
    end
...
sequenceDiagram
    ...
    loop 多轮对话
        ...
        API->>AI: 调用Claude Agent SDK (含上下文)
...
| 模块 | 接口 | 方法 | 说明 |
|------|------|------|------|
| **认证** | `/api/auth/login` | POST | 用户登录（邮箱/手机） |
| **认证** | `/api/auth/verify` | POST | Token验证 |
...
Request:
  ...
  Body:
    ...
    password: string # 密码
    # 或使用快捷登录
    provider?: "google" | "apple" # OAuth提供商
...
### 14.2 完整Schema (Supabase PostgreSQL)
...
CREATE TABLE users (
  ...
  email TEXT UNIQUE,  -- 邮箱登录
  phone TEXT UNIQUE,  -- 手机号登录
  password_hash TEXT,  -- 密码哈希
  device_id TEXT UNIQUE,  -- 游客UUID
  nickname TEXT,
  avatar_url TEXT,
  investment_goals JSONB,  -- ['long_term', 'short_term']
  risk_preference TEXT CHECK (risk_preference IN ('conservative', 'moderate', 'aggressive')),
  preferred_agent TEXT CHECK (preferred_agent IN ('buffett', 'soros', 'munger', 'dalio', 'lynch', 'wood', 'coach')) DEFAULT 'coach',
  is_new_user BOOLEAN DEFAULT true,
  banner_dismissed_count INTEGER DEFAULT 0,
...
CREATE TABLE emotion_logs (
  ...
  emotion TEXT NOT NULL CHECK (emotion IN ('greedy', 'fear', 'calm', 'anxious', 'panic', 'angry')),
  log_date DATE NOT NULL,
  market_status TEXT CHECK (market_status IN ('up', 'down', 'volatile')),
  current_insight_id UUID,
  can_modify BOOLEAN DEFAULT true,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  locked_at TIMESTAMPTZ,
  UNIQUE(user_id, log_date)  -- 每天只能打卡一次
);
...
CREATE TABLE conversations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  agent_type TEXT NOT NULL CHECK (agent_type IN ('buffett', 'soros', 'munger', 'dalio', 'lynch', 'wood', 'coach')),
  strategy_mode TEXT CHECK (strategy_mode IN ('faith', 'discipline')),
  related_symbol TEXT,
  emotion_chip TEXT,
...
CREATE TABLE messages (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  ttl INTEGER DEFAULT 1296000  -- 15天 (秒)
);

CREATE INDEX idx_messages_conversation ON messages(conversation_id, created_at ASC);

-- 自动清理15天前的消息 (使用pg_cron扩展)
...
CREATE TABLE portfolios (
  ...
  broker TEXT,
  strategy TEXT CHECK (strategy IN ('faith', 'discipline')),
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX idx_portfolios_user ON portfolios(user_id);
CREATE INDEX idx_portfolios_ticker ON portfolios(user_id, ticker);
...
CREATE TABLE reviews (
  ...
  emotion_then TEXT CHECK (emotion_then IN ('greedy', 'fear', 'calm', 'anxious', 'panic', 'angry')),
  emotion_detected_by_ai TEXT,
  action_wanted TEXT,  -- 'buy', 'sell', 'hold', 'watch'
  retrospect TEXT,  -- 用户自述
...
-- ========================================
-- 情绪-市场模式表 (emotion_market_patterns)
-- ========================================
CREATE TABLE emotion_market_patterns (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  market_status TEXT NOT NULL,
  emotion TEXT NOT NULL,
  frequency INTEGER DEFAULT 1,
  emotional_consistency NUMERIC(5, 2),  -- 百分比 0-100
  last_occurred_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX idx_patterns_user ON emotion_market_patterns(user_id, market_status);

-- ========================================
-- 熔断日志表 (circuit_breaker_logs)
-- ========================================
...
CREATE INDEX idx_breaker_logs_user ON circuit_breaker_logs(user_id, triggered_at DESC);

-- ========================================
-- RAG知识库表 (knowledge_base) - 使用pgvector
-- ========================================
CREATE EXTENSION IF NOT EXISTS vector;

CREATE TABLE knowledge_base (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  category TEXT NOT NULL,  -- 'investing_wisdom', 'psychology', 'market_data'
  content TEXT NOT NULL,
  metadata JSONB,  -- 存储标签、来源等信息
...
-- ========================================
-- 用户记忆表 (user_memories) - 长期记忆系统
-- ========================================
CREATE TABLE user_memories (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  memory_type TEXT NOT NULL,  -- 'learning', 'preference', 'pattern'
  content TEXT NOT NULL,
  metadata JSONB,
...
CREATE POLICY "Users can update own emotions" ON emotion_logs FOR UPDATE USING (auth.uid() = user_id);
...
CREATE POLICY "Users can insert own memories" ON user_memories FOR INSERT WITH CHECK (auth.uid() = user_id);
### 14.3 数据清理策略
...
**核心实现**: Claude Agent SDK (多Agent协作系统)

系统采用 **Claude Agent SDK** 构建5个专业Agent协作的多智能体系统，通过任务分发与专业分工实现复杂投资心理场景的处理。每个Agent拥有专属工具和模型配置，通过Coordinator智能调度实现无缝协作。

**5个专业Agent概览**:

| Agent | 角色 | 工具 | 模型 | 核心能力 |
|-------|------|------|------|---------|
| **Coordinator** | 协调员 | Task, Read, Grep, AskUserQuestion | Sonnet | 意图理解、任务分发、结果聚合 |
| **Researcher** | 研究员 | Eastmoney API, Sina Finance API, WebSearch | Haiku | 公告抓取、新闻解析、财报提醒 |
| **Analyst** | 分析师 | Market Data, WebSearch, Read, Grep | Sonnet | 技术分析、基本面研究、趋势判断 |
| **Risk Manager** | 风控员 | Market Data, Calculations, WebSearch | Sonnet | 风险评估、仓位管理、止损建议 |
| **Mentor** (7位) | 心理导师 | Read, Grep (RAG) | Sonnet | 情绪疏导、投资哲学、行为引导 |

**Mentor Agent的7位投资导师**:

| 导师 | ID | 人格定位 | 核心能力 | 适用场景 |
|-------|-----|---------|---------|---------|
| 巴菲特 | buffett | 价值投资派 | 长期视角、基本面分析 | 用户过度关注短期波动 |
| 索罗斯 | soros | 反身性理论 | 市场心理、趋势判断 | 市场情绪剧烈波动时 |
| 芒格 | munger | 理性决策派 | 逆向思维、风险控制 | 用户需要冷静决策 |
| 达里奥 | dalio | 原则系统派 | 系统化思考、风险平价 | 建立投资框架 |
| 林奇 | lynch | 成长投资派 | 生活选股、长期持有 | 发现投资机会 |
| 伍德 | wood | 创新科技派 | 颠覆性创新、主题投资 | 科技股投资 |
| 教练 | coach | 心理陪伴派 | 情绪疏导、行为引导 | 需要心理支持 |

**Agent工作流程**:

```
用户消息 → Coordinator Agent (意图分类)
  ├─ 简单问题：直接回答
  ├─ 单一任务：调用单个Agent
  │   ├─ 研究员 (Researcher) - 获取数据
  │   ├─ 分析师 (Analyst) - 技术分析
  │   ├─ 风控员 (Risk Manager) - 风险评估
  │   └─ 导师 (Mentor) - 心理辅导
  └─ 复杂任务：串行调度多个Agent (Researcher → Analyst → Risk Manager → Mentor)
```

**命令系统** (混合模式):
- `/research <symbol>` - 强制调用研究员Agent
- `/analyze <symbol>` - 强制调用分析师Agent
- `/risk <symbol>` - 强制调用风控员Agent
- `/mentor <name>` - 切换导师 (buffett/soros/dalio/munger/lynch/wood/coach)

**MCP Tools工具生态** (9个工具):
- `get_stock_price` - 实时行情 (Sina Finance API)
...
- `calculate_position_value` - 持仓盈亏
- `get_stock_announcement` - 公告查询 (Eastmoney API)

**数据源**:
- **东方财富API** (Eastmoney): A股公告、新闻、财报
- **新浪财经API** (Sina Finance): 实时行情、资金流向
- **FMP API**: 美股/港股数据
...
**核心架构**: Claude Agent SDK + MCP Server (`lib/mcp-tools.ts`)

系统通过MCP (Model Context Protocol) Server实现9个工具，各Agent可根据权限调用这些工具获取实时数据、分析市场情况、管理投资组合。MCP Server采用Zod类型安全架构，确保工具调用的参数验证和类型正确性。

#### 15.3.1 市场数据工具 (9个)

| 工具名称 | 功能 | 参数 | 使用场景 |
|---------|------|------|---------|
| `get_stock_price` | 获取实时股价 | symbol(股票代码) | 用户询问"特斯拉现在多少钱" |
| `get_stock_kline` | 获取K线数据 | symbol, period(日/周/月/年) | "苹果股票最近一个月走势" |
| `get_fund_flow` | 资金流向数据 | symbol | "贵州茅台���金流向" |
| `get_stock_news` | 股票新闻 | symbol | "有关阿里巴巴的最新新闻" |
| `get_market_overview` | 市场概况 | region(A股/美股/港股) | "今天A股怎么样" |
| `get_margin_trading` | 融资融券数据 | symbol | "宁德时代融资情况" |
| `get_earnings_calendar` | 财报日历 | symbol | "特斯拉什么时候发财报" |
| `get_stock_announcement` | **公告查询** (东方财富API) | symbol | "贵州茅台最近的公告" |
| `calculate_position_value` | 持仓盈亏计算 | positions(持仓数组) | "我的特斯拉亏了多少" |

#### 15.3.2 投资组合管理工具 (2个)

| 工具名称 | 功能 | 架构 | 准确率 |
|---------|------|------|---------|
| `parse_portfolio_command` | 解析用户指令 | 三步解析架构 | 85%+ |
| `manage_portfolio` | 执行组合操作 | 调用parse + apply | 完整闭环 |

**Agent智能解析流程**:

```
步骤1: Coordinator Agent接收指令
  ↓ 识别: buy/sell/watch/delete/query

步骤2: 精确解析 (Claude Sonnet, 强推理能力)
  ↓ 提取: 股票代码、数量、价格、策略

步骤3: 验证增强
  ↓ 校验逻辑 + 补全缺失信息

输出: { command, symbol, quantity, price, confidence }
```
...
✅ "我的特斯拉持仓情况"
✅ "500块钱买贵州茅台"  // 自动计算股数
✅ "买100股AAPL,每股150美元" // 多参数解析
```

#### 15.3.3 工作流程

```typescript
// 用户消息 → Claude Agent决策流程
用户: "特斯拉跌了,我很焦虑"
  ↓
Coordinator Agent → 意图分类: "vent" (情绪发泄)
  ↓
调度 Researcher Agent → 获取数据:
  - get_stock_price(symbol="TSLA")
  - calculate_position_value(positions=[用户持仓])
  ↓
工具返回: { price: 245.67, change: -3.2%, pnl: -$320 }
  ↓
调度 Mentor Agent → 心理疏导:
  "看到特斯拉今天跌了3.2%,你的持仓账面亏损320美元,
   这种焦虑是正常的。让我们从长期视角来看..."
```

**安全机制**:
- 所有工具调用都记录日志 (`toolCalls` 数组)
- 最多迭代5次防止无限循环
- 工具执行失败自动降级到纯文本回复
- 敏感操作(买入/卖出)需用户二次确认
...
**API路由**: `/api/portfolio/parse-command` + `/api/portfolio/apply-command`
...
- **输入**: 用户原始消息
- **输出**: `buy | sell | watch | delete | query`
- **耗时**: ~200ms

**Step 2: 精确解析** (`parseWithIntent()`)
- **模型**: Claude Sonnet (强推理能力)
- **输入**: 用户消息 + 意图类型 + 当前持仓
- **输出**: 结构化命令对象
  ```typescript
  {
    ...
    price: 4.0,  // 每股价格
    totalAmount: 400,  // 总金额
    strategy: 'faith' | 'discipline',
    confidence: 95
  }
  ```
- **智能特性**:
  - 自动识别股票代码(支持名称/代码/中英文)
  - 多种价格模式: 总价/单价/市价
  - 策略自动推断(长线→faith, 短线→discipline)
  - 容错纠正(茅台→600519, Apple→AAPL)

**Step 3: 验证增强** (`validateAndEnhance()`)
- 逻辑校验: 数量>0, 价格>0
- 股票代码规范化: 统一格式
- 缺失信息补全: 默认策略、市价填充
- 置信度计算: 综合准确性评分
...
**Fallback机制**:
- 如果GPT解析失败,启用正则表达式解析
- 支持标准格式: "买入{数量}股{股票}@{价格}"
- 容错率: 前两步失败时,fallback准确率60%+

**执行流程** (`/api/portfolio/apply-command`):
```typescript
解析结果 → 验证持仓状态 → 执行操作 → 更新数据库
  ↓
Buy: 新增持仓 / 增加持仓 (加权平均成本)
Sell: 减少持仓 / 清空持仓
Watch: 添加到观望列表
Delete: 删除持仓记录
Query: 返回当前持仓详情
...
| 服务 | 用途 | 数据处理 | 隐私承诺 |
|------|------|--------|--------|
| Claude API | 对话生成 | 内容通过API处理，不用于模型训练 | 已关闭数据保留 |
| Eastmoney API | A股行情/公告 | 获取公开行情数据 | 无个人信息 |
| Sina Finance API | 实时行情 | 获取公开行情数据 | 无个人信息 |
| FMP | 美股/港股行情 | 获取公开行情数据 | 无个人信息 |
...
└─────────────────────────────────────┘
    ↓            ↓            ↓
 Supabase    Claude API   FMP/Eastmoney
(PostgreSQL  (Agent SDK +  (行情数据)
 + pgvector)  MCP Tools)
```
...
| 层级 | 技术 | 版本 | 说明 |
|------|------|------|------|
| **前端框架** | Next.js | 15 | App Router + PWA支持 |
| **UI框架** | React | 18 | 服务端组件支持 |
| **样式** | TailwindCSS | 3.4+ | 原子化CSS |
| **状态管理** | Zustand | 4.x | 轻量级，无样板代码 |
| **后端框架** | Next.js | 15 | App Router + Server Actions |
| **数据库** | Supabase | PostgreSQL 15 | Cloud-native, 自动扩展 |
| **向量搜索** | pgvector | 最新 | RAG知识库支持 |
| **认证** | Supabase Auth | 内置 | 多种登录方式 |
| **AI服务** | Claude | Sonnet & Haiku | Agent SDK + MCP Tools |
| **A股数据** | Eastmoney + Sina | 实时 | 公告、行情、资金流 |
| **美股/港股数据** | FMP | 免费版 | 充足配额 |
| **部署** | Vercel | - | 自动部署，全球CDN |
| **PWA** | next-pwa | 5.x | 离线支持、安装提示 |

**数据库特性**:
- **PostgreSQL**: 关系型数据库，ACID保证
- **pgvector**: 向量相似度搜索，支持RAG
...
```
Budvest-antigravity-2/           # 项目根目录
├── app/                          # Next.js App Router
│   ├── api/                      # API Routes
│   │   ├── auth/                 # 认证相关
│   │   ├── chat/                 # AI对话 (Claude Agent SDK)
│   │   ├── portfolio/            # 投资组合管理
│   │   │   ├── parse-command/   # 三步解析
│   │   │   ├── apply-command/   # 执行操作
│   │   │   └── init/            # 初始化
...
│   ├── review/page.tsx           # 复盘历史
│   └── layout.tsx                # 全局布局
│
├── lib/                          # 核心库
│   ├── claude-agents.ts          # 5个Claude Agent定义
│   ├── mcp-tools.ts              # MCP Server + 9个工具
│   ├── reasoning.ts              # 意图分类与推理
...
│   ├── BottomNav.tsx             # 底部导航
│   ├── Card.tsx                  # 卡片组件
│   ├── CompanionTab.tsx          # 陪���Tab
│   ├── MarketTab.tsx             # 市场Tab
│   ├── PortfolioTab.tsx          # 投资组合Tab
│   └── ProfileTab.tsx            # 个人资料Tab
│
├── public/                       # 静态资源
│   ├── manifest.json             # PWA配置
│   ├── icons/                    # 图标
...
| **稳定性** | 崩溃率 | <0.1% |
| **稳定性** | 接口成功率 | >99% |
| **兼容性** | 浏览器版本 | Chrome 90+, Safari 14+, Firefox 88+ |
| **兼容性** | 移动设备 | iOS 14+, Android 10+ |
| **兼容性** | PWA安装 | 支持主流浏览器PWA安装 |
| **安全性** | HTTPS | 全部API |
